...
def flush_config():
    ...
    """
    try:
        with open(fullconfigpath+configname, 'w') as configfile:    
            config.write(configfile)
    except IOError, e:
        import logging
        log = logging.getLogger('Cloud') #might not work if logging not yet initialized
        log.exception('Could not write %s.', fullconfigpath+configname)
...
try:
    ...
    #this will fail
    config.read(fullconfigpath+configname)

else:
    if newdir:
        os.chmod(newdir[-1], 0700)
        
    if not config.read(fullconfigpath+configname):
        _needsWrite = True
